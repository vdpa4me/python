...
def rename_mp3_files(directory):
    # Regular expression to match files in the format: kids_X_Y.mp3
    pattern = re.compile(r"(kids_\d+)_(\d+)\.mp3")
    ...
    if final_files:
        # Extract "dayX" from the base name
        csv_base = final_files[0][0].split('_')[1]  # Extract "dayX"
